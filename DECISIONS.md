...
| `ignorePlurals` | `false` → `true` | Re-measured exhaustively 2026-09-04 and still rejected, on better evidence than the first pass. Buys **three zero-result queries**: `breweries` 0 → 9, `wineries` 0 → 10, `eateries` 0 → 11, `bakeries` 1 → 3 — the `-y`/`-ies` class at edit distance 3, beyond `minWordSizefor2Typos`. Costs **15 known-item regressions** over all 4,979 distinct names under the app's real geo parameters — `Basil's` → Sweet Basil, `Grace's` → Guard & Grace, `Oceans` → Ocean Prime - Denver, `Mayas` → Maya. No hit count moves: rivals go from typo=1 to typo=0 and join the exact name in one bucket, where `geo` settles the tie five criteria before `exact` — so this is the geo decision's blast radius, and without geo the same sweep shows only 2. Half the damage is possessives, which no language setting separates from plurals. Three corrections to the first pass: typo tolerance was not what absorbed `steakhouses` and `italians` (their `category-query-*` rule fires via `alternatives: true` either way), hit counts alone hid a ranking change on 12 of 22 plural queries, and measuring without geo undercounted the cost sevenfold. The four `-ies` words belong in the `synonyms.json` K12 already needs. |
