...
distance conflict in section 5 is the one place the two personas contradict each
other outright.

## Current state

**54 cases — 42 `pass`, 8 `accepted`, 4 `fail`.** The four failures have **three root
causes** between them, each measured and each recorded on the case. Every case is a
hand-picked query against real records in the extract, written before the index existed.
The roster below is the current verdict. The appendix at the bottom is how it got here and
is **not** a description of the live configuration.

| shape | cases | verdict |
|---|---|---|
| 1. Exact name | K1–K6 | 6 pass |
| 2. Misspelled name | K7–K12, K21 | 7 pass |
| 3. Concatenated name | K13–K16 | 3 pass, 1 accepted — K14 |
| 4. Partial name | K17–K20 | 4 pass |
| 5. Cuisine | D1–D7 | 6 pass, 1 accepted — D6 |
| 6. Ambiguous term | A1–A8 | 3 pass, 5 accepted — A1, A2, A5, A6, A7 |
| 7. Multi-location chain | C1–C6 | 6 pass |
| 8. Empty query | E1–E4 | 3 pass, 1 accepted — E2 |
| 9. Out-of-corpus query | O1–O4 | 1 pass, **3 fail — O2, O3, O4** |
| 10. Geo | G1–G4 | 3 pass, **1 fail — G2** |

### The four failures, and their three causes

**Word removal — O2 `olive garden`, O4 `sushi in tokyo`.** Both are absent from the corpus,
`removeWordsIfNoResults: "lastWords"` drops the unmatched word, and what survives matches
something. O2 returns five restaurants genuinely named `Olive` — real results answering a
different question. O4 returns one genuine sushi restaurant matching `sushi` and `in` with
no typos, and fails on its own second clause: nothing says `tokyo` matched nothing. All four
values of the setting were measured on 2026-09-07; `none` closes both and returns 0 on eight
in-scope known-item queries, including roster case K12. Kept, and the failures kept with it.

**Typo tolerance — O3 `shake shack`.** A separate cause, and the sharpest failure in the
file: `shack` is dropped, then `shake` reaches `Sake`, `Snake`, `Share` and `Shaker` at one
or two edits, so **none of the ten hits contains a word the user typed** and a burger chain
returns sushi bars. `minWordSizefor1Typo: 6` fixes it exactly and breaks K7
`papas bros steakhouse`; not applied.

**The `geo` decision — G2 `nobu` from Denver.** Returns 74146 `Nobuo at Teeter House`
(Phoenix) above the four real Nobu locations. No record is named exactly `Nobu`, so
every candidate scores `exact=0` and `geo` — second in `ranking` — orders them by
distance, and Phoenix is nearer to Denver than San Diego. `exactOnSingleWordQuery:
"word"` fixes it exactly and was measured twice, on 2026-09-03 and again on 2026-09-04
under the new `ranking`: it costs A1, A2 and A6, scoring 30/37 against 32/37. Kept as a
stated cost rather than traded for three regressions.

### Why 8 cases are `accepted` rather than passing

`accepted` means the written expectation is not met and the result is judged defensible on
its own terms, with the reason recorded on the case. Two causes, both measured:

- **`geo` above `exact` and `attribute` — 6 cases: A1 `rye`, A2 `prime`, A5 `santa fe`,
  A6 `union`, A7 `bistro`, D6 `sushi`.** A nearer partial match can outrank an exact name,
  and a near cuisine match can outrank a distant name match. Deliberate: distance is the
  dominant intent signal for a diner choosing where to eat, and a diner who wants a named
  restaurant elsewhere types the city — ten city-qualified queries each return exactly one
  hit at rank 1. Five ranking orders were measured; see DECISIONS.md §3 and the appendix.
- **No lever exists, or the expectation is superseded — 2 cases: K14 `leftbank`,
  E2 empty query.** K14's three `Left Bank` records still hold the top three; nothing in
  `customRanking` can order them without reordering broad queries by name length. E2's
  top-20 expectation was written for a geo-less national default and the empty query is
  now geo-led, which is correct for discovery.

**Nine shapes plus geo, as CLAUDE.md §8 requires** — the geo section exists because the
known-item / distance conflict is the one place the two personas contradict each other
outright.

## How to use this file

- **Status vocabulary.** `untested` — not yet run. `pass` — expectation met.
  `fail` — expectation not met, and something has to change: the configuration, or the
  interface. `accepted` — expectation not met and the behaviour is judged acceptable on
  its own terms, with the reason recorded in the case. The distinction matters: a case is
  `accepted` when the result is defensible and the written expectation was too strict, not
  merely when the fix is expensive.
...
- Cases flagged **⚠ predicted fail** are ones the profiling said were likely to fail
  ...
  2026-09-03, O2 was confirmed, and D6 was confirmed on 2026-09-04 — in the direction the
  case said was right for persona 2, which is why it reads `accepted` rather than `fail`.
...
| K21 | `stone works`, geo = San Antonio Alamo Heights | P1 | The 3 `Stone Werks` records (Lincoln Heights, The Vineyard, The Rim) and nothing else, ordered nearest first. The chain spells itself `Werks`, so `works` is not a slip but what a diner who has only heard the name will type — the sharpest form of persona 1's headline pain. It is also the case where the ranking is fully explained by one criterion: all three score `typo=1`, so `geo` alone separates them into buckets 0, 2 and 3 and `popularity_score` is never consulted. | **pass** — 3 hits, ranked Lincoln Heights (4.5/98), The Vineyard (4.3/87), The Rim (4.2/203). The order matches neither rating nor review count, which is the point: the best-rated leads and the most-reviewed is last, purely on distance. `stone werks` and `stonewerks` return the same three in the same order at `typo=0`; `stone work` leaks to 11 hits, keeping the three on top and adding Stone Brewing and Runner & Stone. From North San Antonio the order inverts to The Vineyard, The Rim, Lincoln Heights — same records, different order, which is a ranking criterion rather than a filter. All three carry distinct `location_label` values (Alamo Heights, North San Antonio, Northwest) and `location_label_ambiguous: false`, so the rows are legible without the distance line. |
...
| C5 | `pappas bros`, geo = Dallas | P2 | 1959 (Dallas) ranks above 1854 (Houston) on the geo-led page. Both sit in `market: Dallas - Fort Worth`, so a market facet does **not** separate them — only `city` or distance does. | **pass** — since the `ranking` change of 2026-09-04. No record is named exactly `pappas bros`, so both score `exact=0`, tie, and `geo` — at position 2 — orders them: 1959 Dallas rank 1, 1854 Houston rank 2. The conflict with A2 that made this `accepted` is gone, because the `aroundPrecision` dial is gone: there is one bucket at 5 km and the two cases no longer compete for its width. A2 is not protected — it is `accepted` — but it no longer costs this case anything. |
...
| O2 | `olive garden` | P1 | **⚠ predicted fail, confirmed 2026-09-03 (6 hits).** Truly absent, but under `removeWordsIfNoResults: lastWords` dropping `garden` leaves `olive`, which matches 6 records (100042 `Olive Press`, 56608 `Olivette`, 111679 `Olive Lucy's Kitchen Table`, 5292 `Bleu Olive`, …). The user asked for a chain and gets unrelated restaurants presented as answers. This is the motivating case for evaluating `allOptional` versus `lastWords` per section 5. | **fail** — 6 hits, not 0. `garden` is dropped and `olive` survives, so five of the six are honest answers to `olive` at `typo=0 exact=1`: 110344 `Olive B's Big Sky`, 100042 `Olive Press`, 5292 `Bleu Olive`, 111679 `Olive Lucy's Kitchen Table`, 51646 `Fig & Olive - Westchester`. The sixth, 150853 `Comic Strip Live Comedy Club`, arrives on `olive` → `Live` at `typo=2`. **An intent failure rather than a noise one:** the results are real and consistent with a word the user typed, they answer a different question than the one asked. All four values of `removeWordsIfNoResults` were measured on 2026-09-07 and none fixes this without costing more — `none` returns 0 here and on eight in-scope known-item queries, K12 among them; see the change log. What would fix it is telling the user the query was narrowed, which is a presentation change and carries no stated pain (CLAUDE.md §1). |
| O3 | `shake shack` | P1 | 0 results. Also absent, but the fallback is harmless here: `shake` matches 0 names, so stripping the last word still yields nothing. Contrast with O2 — the same setting is safe on one query and harmful on the other, which is why the choice needs a case, not a preference. | **fail** — 10 hits, not 0, and **not one of the ten contains the word `shake`**. `shack` is dropped, and typo tolerance on the 5-letter `shake` then produces the entire page: `Sake` at one deletion returns six sushi bars (30343 and 94621 `Hapa Sushi Grill & Sake Bar`, 118489 `Soko Sushi & Sake bar`, 115150 `Ai Sushi Sake Grill`, 117208 `Samurai Blue Sushi and Sake`, 7551 `Yo Sake Downtown Sushi Lounge`), `Snake` at one substitution returns 16483 `Snake River Grill`, `Share` returns 92713 `The Lion's Share`, and `Shaker Heights` returns 25840 `SASA` on `neighborhood` at `typo=2`. A user typing a burger chain gets sushi. **The worst of the three, because two relaxations compound:** word removal first, then typo tolerance on what is left. The cause is therefore not `removeWordsIfNoResults` alone, and it has a remedy — measured 2026-09-07, `minWordSizefor1Typo: 6` returns 0 here exactly and breaks **K7** `papas bros steakhouse` from 2 hits to 0, because `papas` needs the tolerated typo to reach `Pappas`. Net zero on the roster, at the price of stripping typo tolerance from every 4- and 5-character word — the compromise §5 protects. Not applied. |
| O4 | `sushi in tokyo` | P2 | Must not silently return every sushi restaurant in the US. `tokyo` is absent from the corpus entirely; the response must make clear the location constraint could not be honoured. | **fail** — 1 hit, not 0, and the hit itself is sound. `tokyo` is dropped, `sushi in` survives, and 41962 `In the Raw - Bricktown` (Oklahoma City) matches it on **two words with zero typos** — `In` in the name, `Sushi` in `cuisine_tags`, so `words=2 exact=2 typo=0`. It is a genuine sushi restaurant, not junk. **A disclosure failure, and the expectation named it:** the second clause — “the response must make clear the location constraint could not be honoured” — is not met, because nothing on the page says `tokyo` matched nothing. The first clause, 0 results, needs `removeWordsIfNoResults: 'none'`, measured on 2026-09-07 and rejected at the cost of eight in-scope known-item queries; see the change log. |

## 10. Geo — the known-item / distance conflict
...
| G2 | `nobu`, geo = Denver | P1 | 74146 `Nobuo at Teeter House` (Phoenix) and 75256 `Mitsunobu` (Menlo Park) rank **below** the four true Nobu locations. Tests that prefix and substring matches do not outrank the exact brand. | **fail** — 74146 at rank **1** since the `ranking` change of 2026-09-04, previously rank 2. No record is named exactly `Nobu`, so every candidate scores `exact=0` and `geo` orders them: 74146 sits 585 mi from Denver against 833 mi for 16927 `Nobu San Diego`. Distance buys nothing here — no candidate is local — and it gets the brand wrong, which makes this the clearest cost of the change. `exactOnSingleWordQuery: "word"` fixes it exactly and was re-measured under the new `ranking` on 2026-09-04: it still breaks A1, A2 and A6, 30/37 against 32/37, so `attribute` is kept. |
| G3 | `italian`, geo = Denver | P2 | Geo leads. Denver-area Italian restaurants first, but `aroundPrecision` buckets must be coarse enough that `popularity_score` breaks ties inside a bucket — a marginally closer mediocre restaurant must not outrank an excellent one two streets further. | **pass** — since the precision dial replaced the empty-query rule on 2026-09-03. Every query gets the single 5 km bucket, and `italian` returns 10/10 Denver restaurants. Earlier note kept for the record: the index always did this correctly; it was the app that had stopped asking. Sent geo, the query returns all ten top hits in Denver with popularity strictly decreasing inside the bucket (4.688, 4.590, 4.589, 4.589, 4.497). Re-measured 2026-09-04 after the `ranking` changes and the removal of the dial: still 10/10 Denver, at 1–2 mi, with `popularity_score` ordering inside the bucket (4.688, 4.590, 4.589, 4.589, 4.497). The `category-query-cuisine` rule of 2026-09-04 is what guarantees that ordering rather than leaving it to chance: the query words are removed and `cuisine:Italian` applied as a facet filter, so the text criteria leave the comparison and `popularity_score` orders the bucket. The `attribute` demotion that first fixed this was measured and set aside in favour of the rule, so `ranking` stays Algolia's default — from Houston Downtown and New York the same query used to lead with a lower-rated restaurant carrying "italian" in its name. The trade this note used to describe — one discovery case against three known-item cases — is still real: A1, A2 and A6 are `accepted`, not protected, because `geo` sits above `exact`. |
| G4 | `cyclone anaya's`, geo = Pittsburgh (40.4491, -79.9939) | P1 | All 5 Houston records still returned, ~1,900 km away. A known-item query must not be filtered by distance, only ordered by it as a tie-break. | **pass** |
...
| 2026-09-04 | `ranking` — reverted to Algolia's default order, now declared as a decision | `["typo","words","filters","exact","geo","proximity","custom","attribute"]` → `["typo","geo","words","filters","proximity","attribute","exact","custom"]` | the two rows above, both set aside: `exact` above `geo` rejected on the use case, `attribute` below `custom` superseded by the query rules of the row below | **The formula returns to Algolia's default with `geo` at position 2, and stays there.** The category-ordering flaw the `attribute` demotion fixed is handled by the three query rules instead, so `ranking` carries no bespoke order to defend. `geo` above `exact` is the deliberate half: distance is the dominant intent signal in a reservation product, and a diner who wants a named restaurant elsewhere types the city — ten city-qualified queries each return exactly one hit at rank 1. G3 `italian` still returns 10/10 Denver, C5 `pappas bros` still passes (no record is named exactly `pappas bros`, so `geo` orders Dallas above Houston whatever its position relative to `exact`). | **A1 `rye`, A2 `prime` and A6 `union` return to `accepted`.** From Denver, 117067 `Prime` sits at rank 14 of 49 behind `Ocean Prime - Denver`, 105424 `Rye` at rank 3 of 7, 116815 `Union` at rank 25 of 42, 100624 `Bistro` at rank 198 of 208. Recorded on the cases rather than hidden. G2 `nobu` stays `fail`. **This is the order in force in `scripts/settings.json`;** the five measured orders are in DECISIONS.md §3. |
...
| 2026-09-07 | `removeWordsIfNoResults` — **all four values evaluated, none applied** | `lastWords` → `none` / `allOptional` / `firstWords` | **O2** `olive garden`, **O3** `shake shack`, **O4** `sushi in tokyo` — open question 5, the last one left | **`none` closes all three.** O1–O4 all return 0, which is what their expectations ask for. The setting's blast radius is narrow and was verified rather than assumed: of 81 probes, **70 are byte-identical under all four values**, and no known-item, category, chain, geo or empty-query case moves under any of them. The ten city-qualified queries the geo decision rests on are inert — all words match, so nothing is removed. | **`none` costs eight in-scope known-item queries, all of them 0 hits.** The corpus is a sparse national sample and Chicago, Boston, Atlanta and Seattle hold no records at all, so the removal is what keeps a real chain reachable from a city it does not cover: `benihana chicago` 24 → 0, `ruths chris boston` 31 → 0, `melting pot denver` 26 → 0, `nobu seattle` 9 → 0, `pappas bros atlanta` 2 → 0, `mccormick and schmicks` 15 → 0 (`&` → `and`, a §2 spelling pain), `pappas brothers steakhouse` 3 → 0, and roster case **K12 `pappas brothers` 3 → 0** — which passes only because `lastWords` drops `brothers`, exactly as `synonyms.json` records. `allOptional` is uniformly worse (`melting pot denver` 26 → 388, `pappas brothers steakhouse` 3 → 515, O4 1 → 111). `firstWords` drops the **brand** and keeps the city, which is backwards for persona 1 (`benihana chicago` → 1 hit, O2 6 → 33 junk hits); it would score +1 on the roster while degrading O2, so it was rejected as scoreboard-only. **`lastWords` kept, and O2, O3 and O4 stay `fail`.** The setting is not changed, so the three cases are not met and remain open — relabelling them would have recorded a product judgment the behaviour does not support. Natural-language probes (`sushi near me`, `best steakhouse denver`) were also measured but are **not** cited as justification — §9 puts natural-language querying in the roadmap, assigned to Agent Studio. |

| 2026-09-07 | `minWordSizefor1Typo` — **evaluated, not applied** | `4` → `5` / `6` | **O3** `shake shack`, once the measurement above showed its cause is typo tolerance rather than word removal | **`6` closes O3 exactly**, 10 hits → 0. The ten hits contain no word the user typed: `shack` is dropped and the 5-letter `shake` then reaches `Sake` at one deletion (six sushi bars), `Snake` and `Share` at one substitution, and `Shaker Heights` at two. Raising the floor above 5 removes all of them. `5` changes nothing on O3 but collapses A3 `naya` 28 hits → 1 and A4 `kaya` 40 → 1, both still rank 1. | **`6` breaks K7 `papas bros steakhouse`, 2 hits → 0** — `papas` needs the tolerated typo to reach `Pappas`, so a persona-1 misspelling case fails to buy an out-of-corpus one. Net zero on the roster. It also degrades K21 `stone works` 3 hits → 13 and would strip tolerance from every 4- and 5-character word, which is the compromise §5 protects: 59 pairs of distinct names sit at edit distance 1 and 36 records already fall under the floor. **`4` kept.** This is the case §5 asks for before moving the thresholds — it names the pair the change fixes (`shake`/`Sake`) and the pair it puts at risk (`papas`/`Pappas`) — and the answer is no. |

| 2026-09-04 | **synonyms** — `scripts/synonyms.json` created, 4 one-way entries | no synonyms → `breweries`/`brewery`, `wineries`/`winery`, `eateries`/`eatery`, `bakeries`/`bakery` | the four plural queries left unmatched by typo tolerance, measured while evaluating `ignorePlurals` | **Four queries recovered, three of them from zero.** `breweries` 0 → 9 hits, `wineries` 0 → 10, `eateries` 0 → 11, `bakeries` 1 → 3, verified against the live index after the push. These are the `-y`/`-ies` class: the plural sits 3 edits from the corpus form, past `minWordSizefor2Typos: 8`, so no typo setting reaches them. `oneWaySynonym` rather than a two-way pair because the plural appears nowhere in the corpus — `brewery` 8 times in a name plus 3 as a `cuisine_tag`, `winery` 10, `eatery` 11, `bakery` 2, and the four plurals 0 times in any attribute — so the reverse direction would assert something untrue of the data. | **none.** The four singular queries are byte-identical with synonyms on and off (`brewery` 9, `winery` 10, `eatery` 11, `bakery` 6), so the one-way direction does not leak, and 14 cases spanning known-item, category and chain — `rye`, `prime`, `union`, `nobu`, `naya`, `kaya`, `babylon`, `italian`, `steakhouse`, `thai`, `the smith`, `cyclone`, `pappas bros`, `melting po` — return the same hits and the same rank 1 either way. K12 `pappas brothers` deliberately gets no entry: its prediction was already refuted on 2026-09-03 and it passes through `removeWordsIfNoResults`. |
...
1. ~~**The `exact` tie — one lever, five cases.**~~ **Closed 2026-09-03.** A2 fixed by
   ...
   and rejected at net −2. No schema change was needed. See the `exact` tie
   investigation in the appendix. What remains live is narrower: **if `cuisine_tags`
   ever gains another value that equals a restaurant name, A7's failure mode
   reappears.** Two of 128 values
...
3. ~~**`address` in `searchableAttributes`.**~~ **Closed 2026-09-03 — removed.** It was
   never the sole cause of O3: with `address` gone O3 went from 1 hit to 10, because the
   real leak is `removeWordsIfNoResults`, which is question 5. Removing it bought precision
   on 9 cases (`Cafe 21` 7 hits → 2, `kaya` 67 → 40, `thai` 76 → 45) and gave up street
   search, which no stated pain requires. It has its own change-log row and is out of
   CLAUDE.md §5.
4. ~~**The dial cannot serve chain disambiguation and exact-name protection at once.**~~
   **Closed 2026-09-04, though not the way the fourth run first recorded it.** The premise
   was that one `aroundPrecision` had to serve both; it does not, because the dial is gone
   and there is one bucket at 5 km. C5 `pappas bros` passes on its own terms: no record is
   named exactly `pappas bros`, so every candidate ties on `exact` and `geo` orders Dallas
   above Houston. Exact-name protection was **not** bought — promoting `exact` above `geo`
   would have done it and was rejected on the use case, so A1, A2 and A6 stay `accepted`.
   The two goals no longer compete for one parameter; only one of them is met.
5. ~~**`lastWords`, `allOptional` or `none`?**~~ **Closed 2026-09-07 — `lastWords` kept,
   all four values measured.** The premise held: `none` does return 0 on all four
   out-of-corpus queries. So did the suspected cost, and it is larger than the gain —
   `none` returns 0 on eight in-scope known-item queries, including K12 and
   `benihana chicago`, because forgiveness of one unmatched word is exactly what lets a
   real chain stay reachable from a city the sparse corpus does not cover. No value
   dominates, because which word should be dropped depends on the query, not the
   setting. O2 and O4 therefore **stay `fail`**: the expectation is not met and the
   configuration is deliberately not changed. O3 turned out to have a different cause
   altogether — typo tolerance, not word removal — and its own change-log row.
   Full measurement in the change log.

   What is left is narrower and is **not** a setting: filler words. `optionalWords`, or a
   rule stripping a known filler list, would let `best steakhouse denver` keep its
   meaning instead of relying on which end of the query the engine trims. That belongs
   with the §9 natural-language work rather than here.
...
7. **E2** — not yet run. Is `m = 50` right? Three of the top 20 are 5.0-star records
   ...
   judgment, not a bug.
8. ~~**Where does `geo` belong in `ranking`?**~~ **Answered, and the answer moved twice
   before settling.** 2026-09-03: restored to position 2, Algolia's default, which fixed G3
   with zero regressions. 2026-09-04: moved to position 5, below `exact`, which recovered
   A1, A2 and A6 — then reverted the same day, because promoting `exact` inverts the intent
   model the product rests on. **In force: position 2, above `words`, `attribute` and
   `exact`.** The geo-first standard replica was never needed. The live cost is recorded on
   A1, A2 and A6, which are `accepted` rather than passing, and on G2: where no record
   scores `exact`, the nearest prefix match leads, so `nobu` from Denver returns `Nobuo at
   Teeter House`. See DECISIONS.md §3 for all five orders.
9. ~~**K18 and the ambiguous label.**~~ **Closed 2026-09-03.** The index side was already
   correct and both colliding records carry `location_label_ambiguous: true`; what was
   missing was the front end. `formatPlace` in `src/lib/format.js` now appends distance on
   every row and falls back to `address` on a flagged record when no position is known, so
   K18 and C2 both pass. It remains the one hard requirement the UI inherited from the data
   model rather than from a design preference.
...
# Appendix — how it got here

**Nothing below describes the live configuration.** These are the measurement runs in
order, kept because CLAUDE.md §8 requires every settings change to be attributable and
because a rejected option with a number beside it is worth more than a rule with none.
Each run states what moved and what it cost. Where a run's conclusion was later reversed,
the section says so at the top.

For the configuration actually in force, read `CLAUDE.md` §5; for the reasoning in
reference form, `DECISIONS.md`; for the current verdicts, the roster above.
...
typed query ever carries `aroundLatLng`, distance will outrank the name match
...
obligation. `formatPlace` in `src/lib/format.js` now discharges it, appending
...
— and all still open as question 5 in the open questions above.
...
(2 km) above 117067 `Prime` (1,062 km). The dial neutralised distance to prevent that.
...
Order at the end of this run:
`["typo","words","filters","exact","geo","proximity","custom","attribute"]`.

> **⚠ Superseded later the same day — this is not the live order.** Both moves were set
> aside: `exact` above `geo` rejected on the use case, `custom` above `attribute` in favour
> of the query rules of the fifth run. `ranking` was reverted to Algolia's default with
> `geo` at position 2, which is what `scripts/settings.json` holds. The reversal has its own
> change-log row and DECISIONS.md §3 carries the five measured orders. **Everything below
> in this section describes the reverted order**, including the statuses in *What moved*:
> A1, A2 and A6 are `accepted` today, not protected by `exact`.
...
**Cumulative at the end of this run: 43 pass, 5 accepted, 4 fail, of 52.** The roster now
holds 54 cases and stands at **42 pass, 8 accepted, 4 fail** — D7 and K21 were added
afterwards, and A1, A2 and A6 returned to `accepted` when `ranking` was reverted.
